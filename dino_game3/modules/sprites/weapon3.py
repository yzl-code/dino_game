...
class LaserGun(pygame.sprite.Sprite):
    ...
    def shoot(self, dino):
        # Create multiple laser bullets instead of one
        for i in range(3):
            bullet = LaserBullet(dino.rect.midright)
            bullet.rect.x += i * 5  # Offset each bullet slightly for a spread effect
            self.bullets.add(bullet)
...
class WeaponManager(Weapon_base):
    def __init__(self, Weapon_class, Weapon_image, cfg):
        ...
        self.laser_gun = self.Weapon[2](self.Weapon_image[2], position=(600, 1))  # Add a new weapon: laser gun

    def weapon_update(self, screen):
        self.bullt_gun.bullets.update()
        self.bullt_gun.bullets.draw(screen)
        self.laser_gun.bullets.update()
        self.laser_gun.bullets.draw(screen)
        self.laser_gun.update()  # Update the laser gun position
